...
public class PetService {
    ...
    public ItemStack createFoodItem(int tier, int amount) {
        ...
        Material mat = switch (clamped) {
            case 1 -> Material.SWEET_BERRIES;
            case 2 -> Material.GLOW_BERRIES;
            default -> Material.GOLDEN_CARROT;
        };
        ItemStack food = new ItemStack(mat, Math.max(1, amount));
        ItemMeta meta = food.getItemMeta();
        if (meta != null) {
            String name = switch (clamped) {
                case 1 -> "§a🍖 Nourriture de compagnon — §lT1";
                case 2 -> "§9🍖 Nourriture de compagnon — §lT2";
                default -> "§6🍖 Nourriture de compagnon — §lT3";
            };
    ...
    public boolean applyFoodToPet(Player player, String petId, ItemStack foodStack) {
        ...
        try {
            player.setItemOnCursor(foodStack);
    ...
    public void gainPetXP(Player player, int xpAmount) {
        ...
        for (PetData pd : pets.values()) {
            ...
            if (gained > 0) {
                changed = true;
                String rarityColor = switch (def.rarity()) {
                    case COMMON -> "§f";
                    case RARE -> "§5";
                    case EPIC -> "§d";
                    case MYTHIC -> "§6";
