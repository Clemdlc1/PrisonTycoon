...
public class AutominerEnchantGUI {
    ...
    public void handleMenuClick(Player player, int slot, ItemStack clickedItem, ClickType clickType) {
        ...
        // Récupération de l'inventaire (le menu) dans lequel le clic a eu lieu
        Inventory topInventory = player.getOpenInventory().getTopInventory();

        ItemMeta meta = clickedItem.getItemMeta();
        String action = meta.getPersistentDataContainer().get(actionKey, PersistentDataType.STRING);

        // L'UUID de l'automineur est récupéré depuis un item spécifique du menu (ici, au slot 0)
        ItemStack identifierItem = topInventory.getItem(0);
        if (identifierItem == null || identifierItem.getItemMeta() == null) return; // Sécurité
        String autominerUuid = identifierItem.getItemMeta().getPersistentDataContainer().get(autominerUuidKey, PersistentDataType.STRING);
        if (autominerUuid == null) return; // Sécurité
        ...
        if (action != null) {
            if (action.equals("enchant")) {
                ...
                if (clickType.isLeftClick()) {
                    ...
                    levelsToUpgrade = 1000; // Un grand nombre pour maximiser
                }
                ...
                if (levelsToUpgrade > 0) {
                    ...
                    if (autominerData != null) {
                        ...
                        // Mettre à jour l'item dans l'inventaire du joueur
                        ...
                        openEnchantMenu(player, autominerUuid); // Rafraîchir
                    }
                }
        ...
        } else if (topInventory.getItem(12) == null || topInventory.getItem(14) == null) {
            // Gérer l'application d'un cristal
